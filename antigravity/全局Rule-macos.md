...
* **路径处理**：优先使用 `os.path.join` 处理路径，或直接使用正斜杠 `/`。

* **脚本规范**：

* Shell 脚本优先使用 **bash/zsh** (`.sh`)。

* 确保脚本文件具有执行权限（`chmod +x`）。

* 默认编码为 UTF-8，无需额外处理中文兼容性。
